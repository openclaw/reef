...
Permission updates use the existing device signature and replay protection. Concurrent distinct updates take effect in database write order, not signature timestamp order. A successful response acknowledges that write; a later update can immediately supersede it.
