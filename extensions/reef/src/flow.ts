...
export class ReefMessageFlow {
  ...
  async send(peer: string, text: string, context: { thread?: string; replyTo?: string } = {}): Promise<string> {
    const friend = this.options.config.friends[peer];
    if (!friend || friend.safetyNumberChanged) throw new Error(`Reef peer @${peer} is not approved with current keys`);
    const id = this.ulid();
    const result = await composeOutbound({
      ...
      body: { text, ...(context.thread ? { thread: context.thread } : {}), ...(context.replyTo ? { replyTo: context.replyTo } : {}) },
  ...
  async processEntries(entries: InboxEntry[]): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.kind === "receipt") {
        ...
        if (entry.receipt && friend) await confirmDelivery(entry.receipt, friend.ed25519PublicKey, this.options.audit);
        continue;
      }
  ...
  private async processEnvelope(relayPeer: string, envelope: NonNullable<InboxEntry["envelope"]>): Promise<void> {
    ...
    if (budget.notifyOnly) {
      await this.options.onOwnerNotice(`Reef message from @${relayPeer}'s agent: ${result.body.text}`);
    } else {
  ...
  private async loadDelivered(): Promise<void> {
    ...
    try {
      const ids = JSON.parse(await readFile(join(this.options.stateDir, "delivered.json"), "utf8")) as string[];
...
export function createConfiguredGuard(config: ReefChannelConfig, fetcher: typeof fetch = fetch): GuardAdapter {
  if (!config.guard) throw new Error("Reef guard is not configured");
  const apiKey = process.env[config.guard.apiKeyEnv];
  if (!apiKey) throw new Error(`Reef guard credential environment variable ${config.guard.apiKeyEnv} is unset`);
  const options = { apiKey, pinnedModel: config.guard.pinnedModel, timeoutMs: config.guard.timeoutMs, fetch: fetcher };
