...
type Prompt = {
  ...
  text(params: { message: string; initialValue?: string; placeholder?: string; sensitive?: boolean; validate?: (value: string) => string | undefined }): Promise<string>;
...
export const reefSetupAdapter = {
  applyAccountConfig: ({ cfg, input }: { cfg: OpenClawConfig; accountId: string; input: Record<string, unknown> }) => ({
    ...cfg,
    channels: { ...cfg.channels, reef: { ...(cfg.channels?.reef as object ?? {}), ...input, dmPolicy: "pairing" } },
  }) as OpenClawConfig,
};
...
export const reefSetupWizard = {
  ...
  getStatus: async ({ cfg }: { cfg: OpenClawConfig }) => {
    ...
    return { channel: "reef", configured, statusLines: [configured ? `Reef @${parsed.data.handle}` : "Reef not configured"] };
  },
  ...
  configureInteractive: async ({ cfg, prompter }: { cfg: OpenClawConfig; prompter: Prompt }) => {
    ...
    const requestPolicy = await prompter.select({
      ...
      options: [
        { value: "code-only" as const, label: "Code only (recommended)", hint: "Requests need an out-of-band code" },
        { value: "friends-of-friends" as const, label: "Friends of friends" },
        { value: "open" as const, label: "Open", hint: "Anyone knowing the exact handle may request" },
      ],
    });
    ...
    const apiKeyEnv = await prompter.text({ message: "Guard API key environment variable name", initialValue: provider === "anthropic" ? "ANTHROPIC_API_KEY" : "OPENAI_API_KEY" });
    const policyVersion = await prompter.text({ message: "Guard policy version", initialValue: "reef-v1" });
    const reef: ReefChannelConfig = ReefChannelConfigSchema.parse({
      relayUrl, handle, email, requestPolicy, stateDir, friends: {}, dmPolicy: "pairing", allowFrom: [],
    ...
    return { cfg: { ...cfg, channels: { ...cfg.channels, reef } } as OpenClawConfig, accountId: "default" };
  },
};
