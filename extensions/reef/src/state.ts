...
import { base64url, fromBase64url, generateIdentity, type ReviewApproval, type ReviewRequest } from "@openclaw/reef-protocol";
import { JsonlAuditStore, FileReplayStore } from "@openclaw/reef-protocol/node";
...
export async function loadKeys(stateDir: string): Promise<ReefKeys> {
  ...
  if (fromBase64url(value.signing.secretKey).length !== 32 || fromBase64url(value.encryption.secretKey).length !== 32 ||
      fromBase64url(value.auditKey).length !== 32 || fromBase64url(value.replayKey).length !== 32 ||
...
export class ReviewApprovalStore {
  ...
  constructor(stateDir: string) { this.path = join(stateDir, "reviews.json"); }

  async request(review: ReviewRequest): Promise<ReviewApproval | undefined> {
    const records = await this.read();
    const current = records[review.approvalDigest];
    if (current?.approved !== undefined) return { approved: current.approved, approvalDigest: review.approvalDigest };
  ...
  async list(): Promise<ReviewRequest[]> {
    return Object.values(await this.read()).filter((entry) => entry.approved === undefined).map((entry) => entry.review);
  }

  private async read(): Promise<Record<string, { review: ReviewRequest; approved?: boolean }>> {
    try { return JSON.parse(await readFile(this.path, "utf8")) as Record<string, { review: ReviewRequest; approved?: boolean }>; }
