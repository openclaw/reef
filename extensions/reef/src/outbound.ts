...
import type { ChannelOutboundAdapter, OutboundDeliveryResult } from "openclaw/plugin-sdk/channel-send-result";
import { normalizeReefTarget } from "./config-schema.js";
import { getActiveReef } from "./runtime.js";

async function send(to: string, text: string, threadId?: string | number | null, replyToId?: string | null): Promise<OutboundDeliveryResult> {
...
export const reefOutboundAdapter: ChannelOutboundAdapter = {
  ...
  resolveTarget: ({ to }) => {
    const peer = normalizeReefTarget(to ?? "");
    return peer ? { ok: true, to: peer } : { ok: false, error: new Error("Reef target must be a handle") };
  },
...
export const reefMessageAdapter = defineChannelMessageAdapter({
  ...
  receive: { defaultAckPolicy: "after_receive_record", supportedAckPolicies: ["after_receive_record"] },
});
