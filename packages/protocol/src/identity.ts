...
export function generateIdentity(): IdentityKeyPair {
  ...
  return {
    ...
    encryption: { publicKey: base64url(encryption.publicKey), secretKey: base64url(encryption.secretKey) },
  };
}
...
export function fingerprint(ed25519PublicKey: string, x25519PublicKey?: string): string {
  ...
  return hex(sha256(material)).match(/.{1,4}/g)!.join(" ");
}

export function formatHandleEpoch(handle: string, keyEpoch: number): string {
  if (!/^[a-z0-9](?:[a-z0-9_-]{0,62})$/i.test(handle) || !Number.isSafeInteger(keyEpoch) || keyEpoch < 1) {
