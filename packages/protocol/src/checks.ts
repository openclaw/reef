...
export function deterministicChecks(input: string | Uint8Array): CheckResult {
  ...
  if (bytes.length > MAX_BYTES) return { allowed: false, text, findings: [{ code: "too_large", decision: "deny" }] };
  const findings: CheckFinding[] = [];
  for (const [code, pattern] of rules) if (pattern.test(text)) findings.push({ code, decision: "deny" });
...
function hasHighEntropyToken(text: string): boolean {
  ...
  if (hexCandidates.some((candidate) => {
  ...
  return looseCandidates.some((candidate) => /[A-Za-z]/.test(candidate) && /[0-9]/.test(candidate) && shannonEntropy(candidate) >= 4.0);
}
