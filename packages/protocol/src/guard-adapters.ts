import { admitGuardAdapter, assertPinnedModel, INBOUND_INSTRUCTIONS, OUTBOUND_INSTRUCTIONS, type GuardAdapter, type GuardRequest, type RawGuardAdapter } from "./guard.js";
    ...
    async classifyRaw(request, signal) {
      const response = await options.fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          ...
          background: false,
          tools: [],
          text: { format: { type: "json_schema", name: "reef_guard_verdict", strict: true, schema: verdictSchema } },
        }),
      });
      ...
      for (const item of envelope.output) {
        ...
        for (const part of item.content) {
          if (isRecord(part) && part.type === "output_text" && typeof part.text === "string") outputTexts.push(part.text);
        }
      }
...
function instructionFor(request: GuardRequest): string {
  const directionInstructions = request.direction === "outbound" ? OUTBOUND_INSTRUCTIONS : INBOUND_INSTRUCTIONS;
