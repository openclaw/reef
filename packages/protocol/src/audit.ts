...
export class MemoryAuditStore implements AuditStore {
  ...
  async appendEvent(type: string, payload: unknown, ts = Math.floor(Date.now() / 1000)): Promise<AuditEntry> {
...
export function appendAudit(store: AuditStore, type: string, payload: unknown, ts?: number): Promise<AuditEntry> {
  return store.appendEvent(type, payload, ts);
}

export async function appendInboxRead(store: AuditStore, ids: string[], ts?: number): Promise<AuditEntry> {
  return appendAudit(store, "read", { ids }, ts);
}

export function verifyChain(entries: readonly AuditEntry[], expected?: { head?: string; length?: number }): boolean {
  if (expected?.length !== undefined && entries.length !== expected.length) return false;
  let previous = "";
  for (let index = 0; index < entries.length; index++) {
    const entry = entries[index]!;
    if (entry.event.seq !== index + 1 || entry.prevHash !== previous || entry.entryHash !== hashEntry(previous, entry.event)) return false;
...
function encryptSensitive(value: unknown, key: Uint8Array, rng: (length: number) => Uint8Array): unknown {
...
function decryptSensitive(value: unknown, key: Uint8Array, field?: string): unknown {
  ...
  if (value !== null && typeof value === "object") {
    ...
    if ((field === "text" || field === "reason") && Object.keys(record).length === 1 && typeof record.enc === "string") {
    ...
    return Object.fromEntries(Object.entries(record).map(([childField, child]) => [childField, decryptSensitive(child, key, childField)]));
  }
  return value;
}
