...
export type ProtocolErrorCode = "bad_signature" | "not_pinned" | "wrong_recipient" | "expired" | "replayed" | "too_large" | "malformed";

export class ProtocolError extends Error {
  constructor(readonly code: ProtocolErrorCode, message: string = code) {
    super(message);
    this.name = "ProtocolError";
  }
}

export class BadSignatureError extends ProtocolError { constructor(message?: string) { super("bad_signature", message); this.name = "BadSignatureError"; } }
export class NotPinnedError extends ProtocolError { constructor(message?: string) { super("not_pinned", message); this.name = "NotPinnedError"; } }
export class WrongRecipientError extends ProtocolError { constructor(message?: string) { super("wrong_recipient", message); this.name = "WrongRecipientError"; } }
export class ExpiredError extends ProtocolError { constructor(message?: string) { super("expired", message); this.name = "ExpiredError"; } }
export class ReplayedError extends ProtocolError { constructor(message?: string) { super("replayed", message); this.name = "ReplayedError"; } }
export class TooLargeError extends ProtocolError { constructor(message?: string) { super("too_large", message); this.name = "TooLargeError"; } }
export class MalformedError extends ProtocolError { constructor(message?: string) { super("malformed", message); this.name = "MalformedError"; } }
...
export function seal(options: SealOptions): Envelope {
  ...
  return { ...unsigned, sig: base64(ed25519.sign(canonicalBytes(unsigned), decodeKey(options.senderSigningSecretKey))) };
}
...
export function validateMessageBody(value: unknown): asserts value is MessageBody {
  if (!isExactObject(value, ["text", "replyTo", "thread"])) throw new MalformedError("invalid body");
  if (typeof value.text !== "string" || (value.replyTo !== undefined && typeof value.replyTo !== "string") || (value.thread !== undefined && typeof value.thread !== "string")) {
    throw new MalformedError("invalid body");
  }
  if ((value.replyTo !== undefined && !ULID_PATTERN.test(value.replyTo)) || (value.thread !== undefined && !ULID_PATTERN.test(value.thread))) {
    throw new MalformedError("invalid body identifier");
  }
  for (const field of [value.text, value.replyTo, value.thread]) {
    if (field !== undefined && decodeUtf8(utf8(field)) !== field) throw new MalformedError("invalid UTF-8 body");
  }
}

function validateEnvelope(value: unknown): Envelope {
  if (!isExactObject(value, ["v", "id", "from", "to", "ts", "epk", "n", "ct", "sig"])) throw new MalformedError();
  if (typeof value.v !== "number" || typeof value.id !== "string" || typeof value.from !== "string" || typeof value.to !== "string" ||
      !Number.isSafeInteger(value.ts) || typeof value.epk !== "string" || typeof value.n !== "string" || typeof value.ct !== "string" || typeof value.sig !== "string") {
    throw new MalformedError();
  }
  if (value.id.length !== 26) throw new MalformedError("invalid envelope id length");
  if (value.from.length > 80 || value.to.length > 80 || value.epk.length > 46 || value.n.length > 18 ||
      value.sig.length > 90 || value.ct.length > MAX_CIPHERTEXT_BASE64) {
