...
async function openFixture(envelope: Envelope, alice: ReturnType<typeof generateIdentity>, bob: ReturnType<typeof generateIdentity>, self = "bob#1", at = now) {
  return open({
    envelope,
    self,
...
describe("identity", () => {
  it("formats, parses, and fingerprints identities stably", () => {
    ...
    expect(parseHandleEpoch(formatHandleEpoch("reef-bot", 3))).toEqual({ handle: "reef-bot", keyEpoch: 3 });
    expect(fingerprint(identity.signing.publicKey, identity.encryption.publicKey)).toBe(fingerprint(identity.signing.publicKey, identity.encryption.publicKey));
  ...
  it("authenticates planned rotation with the old signing key", () => {
    ...
    const rotation = signRotation({ newEd25519Pub: next.signing.publicKey, newX25519Pub: next.encryption.publicKey, newEpoch: 2 }, oldIdentity.signing.secretKey);
    ...
    const legacyStatement = { newEd25519Pub: next.signing.publicKey, newX25519Pub: next.encryption.publicKey, newEpoch: 2 };
    const legacy = { ...legacyStatement, signature: base64url(ed25519.sign(canonicalBytes(legacyStatement), fromBase64url(oldIdentity.signing.secretKey))) };
    expect(verifyRotation(legacy, oldIdentity.signing.publicKey)).toBe(false);
    expect(verifyRotation({ ...rotation, domain: "attacker-domain" } as typeof rotation, oldIdentity.signing.publicKey)).toBe(false);
  });
});
...
describe("envelope", () => {
  ...
  it("consumes a standalone envelope after one successful open", async () => {
    ...
    await expect(open(options)).rejects.toMatchObject({ code: "replayed", message: "duplicate envelope" });
  ...
  it("verifies every signed field before acting on it", async () => {
    ...
    for (const mutated of mutations) {
      await expect(openFixture(mutated, alice, bob)).rejects.toMatchObject({ code: "bad_signature" });
    }
  });

  it("rejects an unpinned sender", async () => {
    const { bob, envelope } = fixture();
    await expect(open({ envelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey, replayStore: new MemoryReplayStore(), now })).rejects.toMatchObject({ code: "not_pinned" });
  });

  it("rejects the wrong recipient", async () => {
    const { alice, bob, envelope } = fixture();
    await expect(openFixture(envelope, alice, bob, "carol#1")).rejects.toMatchObject({ code: "wrong_recipient" });
  });
  ...
  it("accepts store-and-forward delivery ten minutes later", async () => {
    ...
    const bob = generateIdentity();
    const envelope = seal({
      id, from: "alice#1", to: "bob#1", body: { text: "delayed" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now - 10 * 60,
    });
  ...
  it("rejects envelopes beyond the future clock-skew bound", async () => {
    ...
    const bob = generateIdentity();
    const envelope = seal({
      id, from: "alice#1", to: "bob#1", body: { text: "future" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now + 3_600,
    });
  ...
  it("rejects envelopes older than relay retention", async () => {
    ...
    const bob = generateIdentity();
    const envelope = seal({
      id, from: "alice#1", to: "bob#1", body: { text: "ancient" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now - 40 * 24 * 60 * 60,
    });
    await expect(openFixture(envelope, alice, bob)).rejects.toMatchObject({ code: "expired" });
  });
  ...
  it("permanently binds an id to the first verified envelope hash", async () => {
    ...
    await open({ envelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey, senderSigningPublicKey: alice.signing.publicKey, replayStore: store, now });
    const replacement = seal({ id, from: "alice#2", to: "bob#1", body: { text: "different" }, senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now });
    await expect(open({ envelope: replacement, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey, senderSigningPublicKey: alice.signing.publicKey, replayStore: store, now })).rejects.toMatchObject({ code: "replayed", message: "replay id binding mismatch" });
  });
  ...
  it("namespaces identical envelope ids by authenticated sender handle", async () => {
    ...
    const aliceEnvelope = seal({
      id, from: "alice#1", to: "bob#1", body: { text: "from alice" },
    ...
    const carolEnvelope = seal({
      id, from: "carol#1", to: "bob#1", body: { text: "from carol" },
      senderSigningSecretKey: carol.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now,
    });
    await expect(open({
      envelope: aliceEnvelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey,
      senderSigningPublicKey: alice.signing.publicKey, replayStore, now,
    })).resolves.toEqual({ text: "from alice" });
    await expect(open({
      envelope: carolEnvelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey,
      senderSigningPublicKey: carol.signing.publicKey, replayStore, now,
  ...
  it("atomically admits exactly one concurrent open", async () => {
    ...
    expect(rejected).toMatchObject({ status: "rejected", reason: { code: "replayed", message: "in flight" } });
  });

  it("enforces the plaintext cap", () => {
    const { alice, bob } = fixture();
    expect(() => seal({ id, from: "alice#1", to: "bob#1", body: { text: "x".repeat(33 * 1024) }, senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now })).toThrow(ProtocolError);
  });
