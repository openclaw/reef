...
describe("friend codes", () => {
  it("mints deterministic Crockford codes and enforces expiry", () => {
    ...
    expect(verifyFriendCode({ ...code, code: `Z${code.code.slice(1)}` }, secret, { now: 2_000 })).toBe(false);
  });
  ...
  it.each([0, 16])("rejects a %i-byte device secret", (length) => {
    ...
    expect(() => mintFriendCode(shortSecret, { expiry: 2_000, nonce })).toThrow("invalid friend code input");
