...
export class FileReplayStore implements ReplayStore {
  ...
  async complete(peer: string, id: string, receipt: SignedReceipt, body?: MessageBody): Promise<void> {
    return this.#withLock(async () => {
      await this.#load();
      ...
      const record: ReplayLogRecord = body === undefined
        ? { op: "complete", peer, id, receipt }
        : { op: "complete", peer, id, receipt, body: encryptReplayBody(body, this.#bodyKey, this.#rng) };
  ...
  async #load(): Promise<void> {
    ...
    for (const record of await readJsonl<ReplayLogRecord>(this.path)) {
      if (record.op === "claim") {
        ...
        const existing = this.#bindings.get(key);
        if (existing !== undefined && existing.envelopeHash !== record.envelopeHash) throw new Error("corrupt replay binding store");
        ...
        const body = record.body === undefined ? undefined : decryptReplayBody(record.body, this.#bodyKey);
...
function encryptReplayBody(body: MessageBody, key: Uint8Array, rng: (length: number) => Uint8Array): EncryptedReplayBody {
