...
describe("audit", () => {
  it("builds and verifies a chain and signed checkpoint", async () => {
    ...
    await appendAudit(store, "guard_verdict", { decision: "allow", reason: "looks safe", model: "model-20260101" }, 11);
  ...
  it("detects mutation and externally anchored truncation", async () => {
    ...
    expect(verifyChain([{ ...entries[0]!, event: { ...entries[0]!.event, type: "changed" } }, entries[1]!], expected)).toBe(false);
    expect(verifyChain(entries.slice(0, 1), expected)).toBe(false);
  });

  it("exports an unchanged, verifiable chain without plaintext", async () => {
    const store = new MemoryAuditStore(auditKey);
    await appendAudit(store, "inbox", { id: "one", text: "DO NOT LEAK ME", verdict: { decision: "deny", reason: "ALSO PRIVATE", model: "guard-20260101" } }, 10);
    const entries = await store.entries();
  ...
  it("serializes twenty concurrent appends into one valid chain", async () => {
    const store = new MemoryAuditStore(auditKey);
    await Promise.all(Array.from({ length: 20 }, (_, index) => store.appendEvent("concurrent", { index }, 100 + index)));
    const entries = await store.entries();
    expect(entries).toHaveLength(20);
    expect(entries.map((entry) => entry.event.seq)).toEqual(Array.from({ length: 20 }, (_, index) => index + 1));
...
describe("receipts", () => {
  it("verifies and audits accepted and rejected delivery outcomes", async () => {
    ...
    const accepted = signReceipt({ id: "01JZ0000000000000000000000", bodyHash: "a".repeat(64), auditHead: "b".repeat(64), status: "accepted" }, identity.signing.secretKey);
    const rejected = signReceipt({ id: "01JZ0000000000000000000001", bodyHash: "c".repeat(64), auditHead: "d".repeat(64), status: "rejected", category: "guard_deny" }, identity.signing.secretKey);
    ...
    expect(entries.map((entry) => entry.event.type)).toEqual(["confirm_delivery", "confirm_delivery"]);
    expect(entries.map((entry) => (entry.event.payload as { status: string }).status)).toEqual(["accepted", "rejected"]);
    await expect(confirmDelivery({ ...accepted, bodyHash: "e".repeat(64) }, identity.signing.publicKey, audit)).rejects.toThrow("invalid delivery receipt");
  });
