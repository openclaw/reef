...
export function mintFriendCode(deviceSecret: Uint8Array, options: { expiry: number; nonce?: Uint8Array; rng?: (length: number) => Uint8Array }): FriendCode {
  if (deviceSecret.length < 32) throw new Error("invalid friend code input");
  const nonce = options.nonce ?? (options.rng ?? randomBytes)(16);
  if (!Number.isSafeInteger(options.expiry) || options.expiry < 0 || nonce.length < 8) throw new Error("invalid friend code input");
...
export function verifyFriendCode(code: FriendCode, deviceSecret: Uint8Array, options: { now?: number; clockSkewSeconds?: number } = {}): boolean {
  if (deviceSecret.length < 32) return false;
  try {
    const now = options.now ?? Math.floor(Date.now() / 1000);
    if (code.expiry + (options.clockSkewSeconds ?? 0) < now) return false;
    const expected = mintFriendCode(deviceSecret, { expiry: code.expiry, nonce: fromBase64url(code.nonce) });
    return equalBytes(new TextEncoder().encode(expected.code), new TextEncoder().encode(code.code.toUpperCase()));
  } catch {
    return false;
  }
}
