...
import { bodyHash, openClaimed, ReplayedError, seal, validateEnvelopeMetadata, validateMessageBody, type Envelope, type MessageBody } from "./envelope.js";
import { fromBase64url, hex } from "./encoding.js";
import { admitVerdict, type GuardAdapter, type GuardDirection, type GuardRequest, type Verdict } from "./guard.js";
...
export async function composeOutbound(options: ComposeOutboundOptions): Promise<OutboundResult> {
  ...
  if (checks.findings.some((finding) => finding.code === "invalid_utf8" || finding.code === "too_large")) {
  ...
  await appendAudit(options.audit, "proposal", { id: options.id, from: options.from, to: options.to, bodyHash: proposalHash, approvalDigest, body: options.body });
  if (!checks.allowed) {
    await appendAudit(options.audit, "deterministic_verdict", { id: options.id, approvalDigest, decision: "deny", findings: checks.findings });
...
export async function composeInbound(options: ComposeInboundOptions): Promise<InboundResult> {
  ...
  try {
    ...
    if (!checks.allowed) {
      await appendAudit(options.audit, "deterministic_verdict", { id: options.envelope.id, approvalDigest, decision: "deny", findings: checks.findings });
      const receipt = await completeRejection(options, peer, proposalHash, approvalDigest, "deterministic_deny");
      finalized = true;
      throw new PipelineError("deterministic", "deterministic checks denied message", undefined, receipt);
    }
    let verdict: Verdict;
    try {
    ...
    const inboxEntry = await appendAudit(options.audit, "inbox", { id: options.envelope.id, bodyHash: proposalHash, approvalDigest, text: opened.body.text, verdict });
...
async function classifyWithReview(
  ...
  const request: GuardRequest = { direction, source, destination, text, policyVersion: options.policyVersion };
  let verdict = admitVerdict(await options.guard.classify(request), options.guard.pinnedModel, request.policyVersion);
  await appendAudit(options.audit, "guard_verdict", { id, from: source, to: destination, direction, bodyHash: proposalHash, approvalDigest, ...verdict });
  if (verdict.decision === "deny") throw new PipelineError("guard", "guard denied message", verdict);
  if (verdict.decision === "review") {
    const approval = await options.reviewGate?.({ id, from: source, to: destination, direction, bodyHash: proposalHash, approvalDigest, verdict });
    if (approval === undefined) throw new PipelineError("review", "review approval pending", verdict, undefined, "pending", approvalDigest);
    if (approval.approvalDigest !== approvalDigest) throw new PipelineError("review", "approval digest mismatch", verdict, undefined, "pending", approvalDigest);
    if (!approval.approved) throw new PipelineError("review", "review explicitly denied", verdict, undefined, "denied", approvalDigest);
    await appendAudit(options.audit, "review_approval", { id, from: source, to: destination, direction, bodyHash: proposalHash, approvalDigest, approved: true });
    verdict = admitVerdict(await options.guard.classify(request), options.guard.pinnedModel, request.policyVersion);
    await appendAudit(options.audit, "guard_verdict", { id, from: source, to: destination, direction, bodyHash: proposalHash, approvalDigest, afterApproval: true, ...verdict });
...
function computeApprovalDigest(
  ...
  return hex(sha256(canonicalBytes({ id, from, to, direction, bodyHash: proposalHash, policyVersion })));
}
