...
const allow: Verdict = { decision: "allow", category: "safe", reason: "Safe.", model: "mock-2026-07-12", policyVersion: "v1" };
...
function mockGuard(...verdicts: Verdict[]): GuardAdapter & { calls: number } {
  return {
    ...
    calls: 0,
    async classify() { return verdicts[this.calls++] ?? verdicts.at(-1)!; },
  };
}

function structuralGuard(pinnedModel: string, ...results: unknown[]): GuardAdapter & { calls: number } {
  return {
    providerId: "structural",
    pinnedModel,
    calls: 0,
    async classify() { return (results[this.calls++] ?? results.at(-1)) as Verdict; },
  };
}
...
describe("pipeline", () => {
  it("runs an allowed outbound and inbound exchange end to end", async () => {
    ...
    const outbound = await composeOutbound({
      ...
      ts: now, guard: mockGuard(allow), audit: outboundAudit, policyVersion: "v1",
    });
    const inboundAudit = audit();
    const inboundGuard = mockGuard(allow);
    const replayStore = new MemoryReplayStore();
    const inboundOptions = {
      envelope: outbound.envelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey,
      recipientSigningSecretKey: bob.signing.secretKey, senderSigningPublicKey: alice.signing.publicKey,
      replayStore, now, guard: inboundGuard, audit: inboundAudit, policyVersion: "v1",
    };
    ...
    expect(outboundEntries.map((entry) => entry.event.type)).toEqual(["proposal", "guard_verdict", "envelope"]);
    expect(inboundEntries.map((entry) => entry.event.type)).toEqual(["guard_verdict", "inbox", "receipt"]);
    expect(new Set(outboundEntries.map((entry) => (entry.event.payload as { approvalDigest: string }).approvalDigest)).size).toBe(1);
    expect(new Set(inboundEntries.map((entry) => (entry.event.payload as { approvalDigest: string }).approvalDigest)).size).toBe(1);
    const duplicate = await composeInbound({ ...inboundOptions, now: now + 10 * 60 });
    expect(duplicate).toEqual({ disposition: "duplicate", body: inbound.body, receipt: inbound.receipt });
  ...
  it("stops a deterministic outbound denial before guard and sealing", async () => {
    ...
    const guard = mockGuard(allow);
    let rngCalls = 0;
    await expect(composeOutbound({
      id: "01JZ0000000000000000000000", from: "alice#1", to: "bob#1", body: { text: ["sk-", "abcdefghijklmnopqrstuvwxyz123456"].join("") },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey,
      guard, audit: audit(), policyVersion: "v1", rng(length) { rngCalls++; return new Uint8Array(length); },
  ...
  it("stops an outbound guard denial before sealing", async () => {
    ...
    let rngCalls = 0;
    await expect(composeOutbound({
      id: "01JZ0000000000000000000000", from: "alice#1", to: "bob#1", body: { text: "ordinary text" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey,
      guard: mockGuard(deny), audit: audit(), policyVersion: "v1", rng(length) { rngCalls++; return new Uint8Array(length); },
  ...
  it("centrally rejects invalid verdicts from structural guard adapters", async () => {
    ...
    for (const [name, pinnedModel, rawVerdict] of cases) {
      let rngCalls = 0;
      await expect(composeOutbound({
        id: "01JZ0000000000000000000008", from: "alice#1", to: "bob#1", body: { text: `case ${name}` },
        senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey,
        guard: structuralGuard(pinnedModel, rawVerdict), audit: audit(), policyVersion: "v1",
        rng(length) { rngCalls++; return new Uint8Array(length); },
      })).rejects.toMatchObject({ stage: "guard", verdict: { decision: "deny", category: "guard_failure" } });
      expect(rngCalls).toBe(0);
    }
  });
  ...
  it("accepts a valid structural adapter and admits the post-review verdict again", async () => {
    const { alice, bob } = identities();
    const common = {
      id: "01JZ0000000000000000000009", from: "alice#1", to: "bob#1", body: { text: "structural adapter" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey,
      audit: audit(), policyVersion: "v1",
    };
    await expect(composeOutbound({ ...common, guard: structuralGuard(allow.model, allow) })).resolves.toMatchObject({ verdict: allow });
    const review: Verdict = { ...allow, decision: "review", category: "ambiguous", reason: "Review." };
  ...
  it("rejects an invalid structural inbound verdict instead of accepting it", async () => {
    const { alice, bob } = identities();
    const envelope = seal({
      id: "01JZ0000000000000000000010", from: "alice#1", to: "bob#1", body: { text: "inbound structural adapter" },
    ...
    await expect(composeInbound({
      ...
      guard: structuralGuard(allow.model, { ...allow, policyVersion: "wrong" }), audit: audit(), policyVersion: "v1",
  ...
  it("requires exact full-proposal approval and fresh classification for review", async () => {
    ...
    await expect(composeOutbound({ ...common, audit: audit(), guard: mockGuard(review) })).rejects.toMatchObject({ stage: "review" });
    await expect(composeOutbound({
      ...common,
      audit: audit(),
    ...
    await expect(composeOutbound({ ...common, audit: audit(), guard: mockGuard(review), reviewGate: async () => ({ approved: true, approvalDigest: "wrong" }) })).rejects.toMatchObject({ stage: "review" });
    const guard = mockGuard(review, allow);
    const result = await composeOutbound({ ...common, audit: audit(), guard, reviewGate: async ({ approvalDigest }) => ({ approved: true, approvalDigest }) });
  ...
  it("does not reuse an approval across recipients", async () => {
    ...
    const review: Verdict = { ...allow, decision: "review", category: "ambiguous", reason: "Review." };
    const common = {
      id: "01JZ0000000000000000000007", from: "sender#1", body: { text: "identical body" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey,
      policyVersion: "v1",
    };
    let vincentDigest = "";
    await expect(composeOutbound({
      ...
      reviewGate: async (request) => {
        ...
        expect(request).toMatchObject({ id: common.id, from: common.from, to: "vincent#1", direction: "outbound" });
  ...
  it("releases a replay claim after transient audit failure and retries successfully", async () => {
    const { alice, bob } = identities();
    const envelope = seal({
      id: "01JZ0000000000000000000001", from: "alice#1", to: "bob#1", body: { text: "retry me" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now,
    });
    const replayStore = new MemoryReplayStore();
    ...
    const options = {
      envelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey,
      recipientSigningSecretKey: bob.signing.secretKey, senderSigningPublicKey: alice.signing.publicKey,
      replayStore, now, guard: mockGuard(allow), audit: inboundAudit, policyVersion: "v1",
    };
  ...
  it("returns an identical cached rejection receipt on guard-deny redelivery", async () => {
    const { alice, bob } = identities();
    const envelope = seal({
      id: "01JZ0000000000000000000002", from: "alice#1", to: "bob#1", body: { text: "classify me" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now,
    });
    const replayStore = new MemoryReplayStore();
    const inboundAudit = audit();
    ...
    const options = {
      envelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey,
      recipientSigningSecretKey: bob.signing.secretKey, senderSigningPublicKey: alice.signing.publicKey,
      replayStore, now, guard, audit: inboundAudit, policyVersion: "v1",
    };
  ...
  it("completes explicit inbound review denial and caches its receipt", async () => {
    const { alice, bob } = identities();
    const envelope = seal({
      id: "01JZ0000000000000000000005", from: "alice#1", to: "bob#1", body: { text: "review me" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now,
    });
    const review: Verdict = { ...allow, decision: "review", category: "ambiguous", reason: "Review." };
    ...
    const options = {
      ...
      recipientSigningSecretKey: bob.signing.secretKey, senderSigningPublicKey: alice.signing.publicKey,
      replayStore, now, guard, audit: inboundAudit, policyVersion: "v1",
      reviewGate: async ({ approvalDigest }: { approvalDigest: string }) => ({ approved: false, approvalDigest }),
    };
    let rejection: PipelineError | undefined;
    try {
      await composeInbound(options);
    ...
    const entryCount = (await inboundAudit.entries()).length;
    await expect(composeInbound(options)).resolves.toEqual({ disposition: "duplicate", receipt: rejection!.receipt });
    expect((await inboundAudit.entries()).length).toBe(entryCount);
    expect(guard.calls).toBe(1);
  });
  ...
  it("releases pending inbound review and accepts a later approved retry", async () => {
    const { alice, bob } = identities();
    const envelope = seal({
      id: "01JZ0000000000000000000006", from: "alice#1", to: "bob#1", body: { text: "decide later" },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now,
    });
    const review: Verdict = { ...allow, decision: "review", category: "ambiguous", reason: "Review." };
    ...
    const options = {
      envelope, self: "bob#1", recipientEncryptionSecretKey: bob.encryption.secretKey,
      recipientSigningSecretKey: bob.signing.secretKey, senderSigningPublicKey: alice.signing.publicKey,
      replayStore: new MemoryReplayStore(), now, guard, audit: audit(), policyVersion: "v1",
      reviewGate: async ({ approvalDigest }: { approvalDigest: string }) => decided ? { approved: true, approvalDigest } : undefined,
    };
    await expect(composeInbound(options)).rejects.toMatchObject({ stage: "review", reviewOutcome: "pending", receipt: undefined });
    decided = true;
    await expect(composeInbound({ ...options, now: now + 10 * 60 })).resolves.toMatchObject({ disposition: "accepted", body: { text: "decide later" } });
  ...
  it("completes deterministic inbound denial with a signed rejection", async () => {
    const { alice, bob } = identities();
    const envelope = seal({
      id: "01JZ0000000000000000000003", from: "alice#1", to: "bob#1", body: { text: ["sk-", "abcdefghijklmnopqrstuvwxyz123456"].join("") },
      senderSigningSecretKey: alice.signing.secretKey, recipientEncryptionPublicKey: bob.encryption.publicKey, ts: now,
    });
    const guard = mockGuard(allow);
    await expect(composeInbound({
...
function craftEnvelope(
  ...
  const shared = x25519.getSharedSecret(ephemeral.secretKey, fromBase64url(bob.encryption.publicKey));
